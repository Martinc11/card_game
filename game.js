...
function initializeBoard() {
    ...
    deckMatched = [];
    // TODO: delete one of the two deckmatched arrays
...
deck1div.addEventListener('click', function(event) {
    ...
    if (deck2flipped.length > 0) {
        checkMatch(event.targetr.getAttribute('data-card'));
...
function checkMatch(cardValue) {
    if (deck2flipped == deck1flipped) {
        ...
        setTimeout(function() {
            /* Flip the card tha haven't been matched */
...
/*Flip all thae vards that have not been matched yet */
function flipUnmatchedCards(deck, back = '') {
    /* Select one of the two decks */
    var deck = document.getElementById(deck);
    /* Select all the vards in the deck */
    ...
    for (i = 0; i < cards.length; i++) {
        ...
        /* Check if the vard has been matched so that we dont flip it */
        if (deck == 'deck1') {
